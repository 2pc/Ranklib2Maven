...
public class LambdaMART extends Ranker {
	...
	public void init()
	{
		...
		//Create a table of candidate(候选) thresholds (for each feature). Later on, we will select the best tree split from these candidates 
		...
		for(int f=0;f<features.length;f++)
		{
			...
			else
			{
				...
				thresholds[f] = new float[nThreshold+1];//阀值，临界值，
				thresholds[f][0] = fmin;
	...
	public void loadModelFromString(String modelString)
	{
		try {
//			String content = "";
//			String model = "";
...
//			in.close();
//			//load the ensemble
			ensemble = new Ensemble(modelString);
		}
		catch(Exception ex)
		{
			System.out.println("Error in LambdaMART::load(): " + ex.toString());
	...
	protected void computePseudoResponses(int start, int end, int current)
	{
		...
		for(int i=start;i<=end;i++)
		{
			...
			float[][] changes = computeMetricChange(i, current);/**计算swap(i,j)后变化的ndcg*/		
			...
			for(int j=0;j<r.size();j++)
			{
				...
				for(int k=0;k<r.size();k++)
				{
					...
					if(p1.getLabel() > p2.getLabel())
					{
						...
						lambdas[j] += lambda;
						lambdas[k] -= lambda;
						double delta = rho * (1.0 - rho) * deltaNDCG;// 二阶导数
						weights[j] += delta;
						weights[k] += delta;
					}
				}
